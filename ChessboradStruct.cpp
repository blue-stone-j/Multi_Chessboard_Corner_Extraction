...
int ChessboradStruct::directionalNeighbor(int idx, cv::Vec2f v, cv::Mat chessboard,
                                          Corners &corners, int &neighbor_idx, float &min_dist)
{
...
float ChessboradStruct::chessboardEnergy(cv::Mat chessboard, Corners &corners)
{
  ...
  for (int i = 0; i < chessboard.rows; i++)
  {
    for (int j = 0; j < chessboard.cols - 2; j++)
    {
      ...
      // select corner of chessboard that is far from line
...
void ChessboradStruct::assignClosestCorners(std::vector<cv::Vec2f> &cand, std::vector<cv::Vec2f> &pred,
                                            std::vector<int> &idx)
{
...
void ChessboradStruct::chessboardsFromCorners(Corners &corners, std::vector<cv::Mat> &chessboards, float lamda)
{
  ...
  for (int i = 0; i < corners.p.size(); i++)
  {
    ...
    while (true)
    {
      ...
      for (int j = 0; j < 4; j++)
      {
        proposal[j] = growChessboard(chessboard, corners, j); // add A corner to current chessboard
    ...
    } // end while: have find all potential corners that locate on same board with this corner
    ...
    else // combine two chessboard
    {
      ...
      // update chess board vector
      if (flagpush == true)
      {
        // remove overlap chessboard
        ...
        // add current chess board
        chessboards.push_back(chessboard);
      }

    } // endif: have process this chessboard, push or combine

    // endif
  } // end for: have processed all corners
}
...
void ChessboradStruct::drawchessboard(cv::Mat img, Corners &corners, std::vector<cv::Mat> &chessboards, char *title, int t_, cv::Rect rect)
{
  ...
  std::cout << BOLDBLUE << s << std::endl
            << BOLDGREEN << s1 << std::endl;
